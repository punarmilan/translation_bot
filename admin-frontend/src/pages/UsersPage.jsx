...
export default function UsersPage() {
  ...
  const view = async (user) => {
    setSelected(user); setEditing(false);
    try { setActivity(await getUserActivity(user.user_id)); } catch { setActivity(null); }
  };
  const save = async () => {
  ...
  const addUser = async () => {
    ...
    setCreating(false); setNewUser(blankUser); setMessage("User created"); load();
  };
  ...
  return (
    <>
      ...
      <section className="admin-table-panel">
        ...
        <tbody>{data.items.map((user) => <tr key={user.user_id}><td><div className="admin-user-cell"><span className="admin-avatar">{(user.name || "U")[0]}</span><div><strong>{user.name}</strong><small>{user.email}</small></div></div></td><td>{user.role}</td><td>{user.preferred_language.toUpperCase()}</td><td>{user.meetings_joined}</td><td><StatusBadge value={user.status} /></td><td>{user.created_at ? new Date(user.created_at).toLocaleDateString() : "-"}</td><td><div className="admin-row-actions"><button title="View" onClick={() => view(user)}><Eye size={15} /></button><button title="Edit" onClick={() => { setSelected(user); setEditing(true); setActivity(null); }}><Edit3 size={15} /></button>{user.status === "disabled" ? <button title="Activate" onClick={() => act(user, "activate")}><UserRoundCheck size={15} /></button> : <button title="Disable" onClick={() => act(user, "disable")}><UserRoundX size={15} /></button>}<button title="Promote" onClick={() => act(user, "promote")}><ShieldCheck size={15} /></button><button title="Reset password" onClick={() => act(user, "reset-password")}><KeyRound size={15} /></button><button title="Delete" className="is-danger" onClick={() => act(user, "delete")}><Trash2 size={15} /></button></div></td></tr>)}</tbody></table></div>
      ...
      {selected && <div className="admin-modal-backdrop" onMouseDown={() => setSelected(null)}><section className="admin-modal" onMouseDown={(e) => e.stopPropagation()}><header><div><span>User record</span><h2>{selected.name}</h2></div><button onClick={() => setSelected(null)}><X /></button></header><label>Name<input disabled={!editing} value={selected.name} onChange={(e) => setSelected({ ...selected, name: e.target.value })} /></label><label>Email<input disabled value={selected.email} /></label><label>Role<select disabled={!editing} value={selected.role} onChange={(e) => setSelected({ ...selected, role: e.target.value })}><option>admin</option><option>host</option><option>participant</option></select></label>{selected.role === "admin" && <label>Admin role<select disabled={!editing} value={selected.admin_role || "administrator"} onChange={(e) => setSelected({ ...selected, admin_role: e.target.value })}><option value="administrator">Administrator</option><option value="support">Support</option><option value="content_editor">Content editor</option></select></label>}<label>Preferred language<input disabled={!editing} value={selected.preferred_language} onChange={(e) => setSelected({ ...selected, preferred_language: e.target.value })} /></label>{activity && <div className="admin-record-summary"><strong>Meeting history</strong><span>{activity.meeting_history.length} recent meetings</span><strong>Translation usage</strong><span>{activity.translation_usage.total_translation_events} translation events</span></div>}{editing && <button className="admin-button admin-button--primary" onClick={save}>Save changes</button>}</section></div>}
      {creating && <div className="admin-modal-backdrop" onMouseDown={() => setCreating(false)}><section className="admin-modal" onMouseDown={(e) => e.stopPropagation()}><header><div><span>New account</span><h2>Add user</h2></div><button onClick={() => setCreating(false)}><X /></button></header><label>Name<input value={newUser.name} onChange={(e) => setNewUser({ ...newUser, name: e.target.value })} /></label><label>Email<input type="email" value={newUser.email} onChange={(e) => setNewUser({ ...newUser, email: e.target.value })} /></label><label>Temporary password<input type="password" value={newUser.password} onChange={(e) => setNewUser({ ...newUser, password: e.target.value })} /></label><label>Role<select value={newUser.role} onChange={(e) => setNewUser({ ...newUser, role: e.target.value })}><option>participant</option><option>host</option><option>admin</option></select></label><label>Preferred language<input value={newUser.preferred_language} onChange={(e) => setNewUser({ ...newUser, preferred_language: e.target.value })} /></label><label>Voice preference<select value={newUser.voice_preference} onChange={(e) => setNewUser({ ...newUser, voice_preference: e.target.value })}><option>auto</option><option>feminine</option><option>masculine</option><option>neutral</option></select></label><button className="admin-button admin-button--primary" onClick={addUser}>Create user</button></section></div>}
    </>
  );
}
