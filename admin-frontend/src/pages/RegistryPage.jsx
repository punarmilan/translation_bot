...
export default function RegistryPage({ module, eyebrow, title, description, canCreate = true, canDelete = true }) {
  ...
  const load = () => getModule(module).then((data) => setItems(data.items || []));
  ...
  const remove = async (item) => {
    ...
    await deleteModuleItem(module, item._id); load();
  };
  ...
  return <>
    ...
    {filtered.length === 0 ? <EmptyState /> : <section className="admin-table-panel"><div className="admin-table-scroll"><table><thead><tr><th>Name</th><th>Key</th><th>Status</th><th>Description / Value</th><th>Updated</th><th>Actions</th></tr></thead><tbody>
